...
public class WeatherService {
    ...
    pastWeather(Location loc,
                           LocalDate from, LocalDate to, int period) {
        return  api
                .pastWeather(loc.getLatitude(),
                                loc.getLongitude(),
                                from, to, period)
                .thenApply(s ->
                    s.map(this::dtoToWeatherInfo));
    }
    ...
    public CompletableFuture<Stream<Location>> search(String query) {
        return api.search(query)
            .thenApply(s ->
                s.map(this::dtoToLocation));
    }

    public CompletableFuture<Stream<DayInfo>>
